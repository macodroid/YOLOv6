...
from YOLOv6.yolov6.assigners.atss_assigner import ATSSAssigner
from YOLOv6.yolov6.assigners.tal_assigner import TaskAlignedAssigner
from YOLOv6.yolov6.utils.figure_iou import IOUloss
from YOLOv6.yolov6.utils.general import dist2bbox, bbox2dist, xywh2xyxy
...
class ComputeLoss:
    ...
    def __init__(self,
        ...
        self.predict_fub = predict_fub
        self.fub_loss = nn.MSELoss().cuda()
    ...
    def __call__(
        ...
        feats, pred_scores, pred_distri, pred_fub = outputs
        ...
        loss_fub = self.fub_loss(pred_fub, target_fub)

        loss = self.loss_weight['class'] * loss_cls + self.loss_weight['iou'] * loss_iou + self.loss_weight[
            'dfl'] * loss_dfl + self.loss_weight['fub'] * loss_fub

        return loss, \
            torch.cat(((self.loss_weight['iou'] * loss_iou).unsqueeze(0),
                       (self.loss_weight['dfl'] * loss_dfl).unsqueeze(0),
                       (self.loss_weight['class'] * loss_cls).unsqueeze(0),
                       (self.loss_weight['fub'] * loss_fub).unsqueeze(0))).detach()
