...
from YOLOv6.yolov6.assigners.anchor_generator import generate_anchors
from YOLOv6.yolov6.utils.general import dist2bbox
...
class Detect(nn.Module):
    ...
    def __init__(self, num_classes=80, anchors=None, num_layers=3, inplace=True, head_layers=None, use_dfl=True,
                 reg_max=16, pred_fub=True):  # detection layer
        super().__init__()
        ...
        self.pred_fub = pred_fub

        # Init decouple head
        self.stems = nn.ModuleList()
        self.cls_convs = nn.ModuleList()
        self.reg_convs = nn.ModuleList()
        self.fub_convs = nn.ModuleList()
        ...
        if self.pred_fub:
            for i in range(num_layers):
                idx = i * 2
                self.fub_convs.append(head_layers[idx + 21])
                self.fub_preds.append(head_layers[idx + 22])
    ...
    def forward(self, x):
        if self.training:
            ...
            for i in range(self.nl):
                ...
                # fub prediction
                fub_feat = self.fub_convs[i](fub_x)
                fub_output = self.fub_preds[i](fub_feat)
                ...
                fub_pred_list.append(fub_output.flatten(2).permute((0, 2, 1)))
            ...
            fub_pred_list = torch.cat(fub_pred_list, axis=1)

            return x, cls_score_list_ab, reg_dist_list_ab, cls_score_list_af, reg_dist_list_af, fub_pred_list
            ...
            for i in range(self.nl):
                ...
                fub_pred_list.append(fub_output.reshape([b, 1, l]))
            ...
            fub_pred_list = torch.cat(fub_pred_list, axis=-1).permute(0, 2, 1)
            ...
            return torch.cat(
                ...
                axis=-1), fub_pred_list


def build_effidehead_layer(channels_list, num_anchors, num_classes, reg_max=16, num_layers=3, pred_fub=True):
