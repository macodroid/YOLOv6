...
class Detect(nn.Module):
    ...
    def __init__(self, num_classes=80, anchors=1, num_layers=3, inplace=True, head_layers=None, use_dfl=True,
        ...
        self.centers_convs = nn.ModuleList()
        self.cls_preds = nn.ModuleList()
        self.reg_preds = nn.ModuleList()
        self.centers_preds = nn.ModuleList()
    ...
    def initialize_biases(self):
        ...
        for conv in self.centers_preds:
            b = conv.bias.view(-1, )
            b.data.fill_(1.0)
            conv.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)
            w = conv.weight
    ...
    def forward(self, x):
        if self.training:
            ...
            for i in range(self.nl):
                ...
                # centers
                centers_feat = self.centers_convs[i](centers_x)
                centers_output = self.centers_preds[i](centers_feat)
            ...
            for i in range(self.nl):
                ...
                centers_list.append(centers_output.reshape([b, 1, l]))

            cls_score_list = torch.cat(cls_score_list, axis=-1).permute(0, 2, 1)
            reg_dist_list = torch.cat(reg_dist_list, axis=-1).permute(0, 2, 1)
            centers_list = torch.cat(centers_list, axis=-1).permute(0, 2, 1)
